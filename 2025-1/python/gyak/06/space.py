...
class EntityFactory:
    ...
    def spawn_powerup():
        ...
        powerup_type = random.choice(EntityFactory.POWERUP_TYPES)
        return powerup_type(x, y)
...
class Game:
    ...
    def check_collisions(self):
        for enemy in self.enemies[:]:
            for bullet in self.bullets[:]:
                if enemy.rect.colliderect(bullet.rect): #ha ütköznek
    ...
    def render_entities(self):
        ...
        for entity in self.bullets + self.enemies + self.powerups: # mindig újrarajzolunk mindent
            entity.draw(screen)
    ...
    def handle_player_alive(self):
        if self.player.lives <= 0:
            self.state.game_over(screen, font)
            self.running = False
