...
--}}
    <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
        {{-- Az enctype="multipart/form-data" szükséges, ha fájlt szeretnénk feltölteni az űrlapon keresztül. Ez lehetővé teszi, hogy a fájl adatai is elküldésre kerüljenek a szerverre.": ha hiányzik, akkor nem megy át a fájl, akkor fájlt nem, csak szöveget küld át. A multipart/form-data lehetővé teszi, hogy az űrlap fájlokat (pl. képeket) is elküldjön a szervernek. enctype: Meghatározza, hogyan csomagolja össze a böngésző az űrlap adatait elküldéskor. multipart/form-data: képes bináris adatot küldeni (pl. kép), az adatokat részekre (multipart) bontja --}}
        ...
        <select name="author_id">
            @foreach ($users as $user)
                <option value="{{ $user->id }}"{{ old('author_id', -1) == $user->id ? 'selected' : '' }}>
                    {{ $user->name }}</option>
            @endforeach
        </select><br>
        Publikus? <input type="checkbox" name="is_public" {{ old('is_public') ? 'checked' : '' }}><br>
