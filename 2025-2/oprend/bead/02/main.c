...
void hegybiro_handler(int signum)
{
    if (signum == SIGUSR1)
    {
        ready_count++;
...
void hegybiro_munka()
{
    signal(SIGUSR1, hegybiro_handler);
    signal(SIGUSR2, hegybiro_handler);
    ...
    if (child1 == 0)
    {
        // megsemmisito biztos "sith nagyúr"
        close(p1[1]);
        close(p2[0]);
        close(p2[1]);
        ...
        CsoAdat sz;
        while (read(p1[0], &sz, sizeof(CsoAdat)) > 0)
        {
            printf("[Megsemmisito bizt.] Tisztelt %s hegykozseg, %s, %s tabla fomernoke! Az orszagos hegybiro utasitasara, az aranyszinu sargasag betegseg vedekezes kereteben, azonnal kerem megsemmisiteni a tablat.\n", sz.termohely, sz.tabla, sz.fajta);
            sleep(1);
        }
        close(p1[0]);
        kill(getppid(), SIGUSR2); // megsemmisitve jelzes
        exit(0);
    }
    ...
    if (child2 == 0)
    {
        ...
        CsoAdat sz;
        while (read(p2[0], &sz, sizeof(CsoAdat)) > 0)
        {
            printf("[Vedekezesi bizt.] Tisztelt %s hegykozseg, %s, %s tabla fomernoke! Indul a tavaszi nagy orszagos vedekezes, helyben keri, hogy semmilyen permetezest, vedekezest ne inditsanak.\n", sz.termohely, sz.tabla, sz.fajta);
            sleep(1);
        }
        close(p2[0]);
        kill(getppid(), SIGUSR2); // vedekezes megvan jelzes
        exit(0);
    }
    ...
    {
        CsoAdat adat;
        strncpy(adat.termohely, adatok[i].termohely, 99);
        adat.termohely[99] = '\0';
        strncpy(adat.tabla, adatok[i].tabla, 99);
        adat.tabla[99] = '\0';
        strncpy(adat.fajta, adatok[i].fajta, 99);
        adat.fajta[99] = '\0';
        adat.terulet = adatok[i].terulet;
        adat.pusztitas = adatok[i].pusztitas;

        if (adat.pusztitas >= 30)
        {
            write(p1[1], &adat, sizeof(CsoAdat));
        }
        else
        {
            write(p2[1], &adat, sizeof(CsoAdat));
        }
    }
    ...
    printf("\nTisztelt Mezogazdasagi Miniszter!\nA hegybiro jelenti: A vedekezes es a megsemmisites sikeresen befejezodott minden allomason.\n");
    exit(0);
}
