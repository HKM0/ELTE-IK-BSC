...
notNull :: [a] -> Bool
notNull (_:_) = True    -- notNull (1:[])
notNull _ = False       -- notNull []
...
--remove2nd [1,2] == [1,2]
remove2nd :: [a] -> [a]
remove2nd (x:y:z:xs) = x:z:xs --pl.: 1:2:3:[] == [1,2,3]
remove2nd xs = xs

--f[x:y:z:xs] mi illeszkedik erre?
-- [[1,2,3]]
...
onlySingleton :: [[a]] -> [[a]]
--onlySingleton xs = [ [x] | [x] <- xs]
onlySingleton xs = [ x | x <- xs, isSingleton x]
