...
data Colour = RGB Integer Integer Integer deriving (Show, Eq)
...
data FruitBatch = FruitBatch Fruit Integer deriving (Show, Eq)
...
sumDifferentFruits xs = help xs (0,0,0) where
    help [] sums = sums
    help (FruitBatch Apple n:xs) (a,g,p) = help xs (a+n,g,p)
    help (FruitBatch Grape n:xs) (a,g,p) = help xs (a,g+n,p)
    help (FruitBatch Pear n:xs) (a,g,p) = help xs (a,g,p+n)
