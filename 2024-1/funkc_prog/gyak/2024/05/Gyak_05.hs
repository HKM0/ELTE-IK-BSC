...
numWords :: String ->  [(Integer, String)]
numWords x = zip [1..] (words x)

numberWords' :: Num a => String ->  [(a, String)]
numberWords' x = numWords (words x) 1 where
  numWords [] _ = []
  numWords (x:xs) a = (a, x) : numWords xs (a+1)
...
intersperse' :: a -> [a] -> [a]
intersperse' e [] = []
intersperse' e (x:xs) = x : interHelp e xs where
  interHelp e [] = []
  interHelp e (x:xs) = e : x : interHelp e xs
...
showList' [] = "[]"
showList' (x:xs) = "[" ++ show x ++ showRest xs ++ "]" where
  showRest [] = "" 
